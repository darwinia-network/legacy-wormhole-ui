...
import { useAllTokens, useCancelablePromise, useLocalSearch } from '../../hooks';
import {
  confirmRegister,
...
import { getNameAndLogo, getSymbolAndDecimals, getTokenName } from '../../page/CrossChain/erc20/token-util';
import {
  config,
  connectNodeProvider,
  getUnitFromValue,
...
export default function Erc20Token(props) {
  ...
  const facade = [
    {
      ...
      icon: null,
      content: <SearchToken onSelect={props.onHide} networkType={networkType} />,
      footer: (
        <Button
          onClick={() => {
            setStep(1);
  ...
  return (
    ...
    >
      ...
      {networkType === 'eth' ? <Modal.Footer>{facade[step].footer}</Modal.Footer> : null}
    </Modal>
  );
}
...
function ListItem(props) {
  ...
  return (
    ...
    >
      <div>
        {token.logo ? <img src={`/images/${logo}`} alt="" /> : <JazzIcon address={source || address}></JazzIcon>}
        <div>
    ...
    <>
      ...
      {loading ? (
        ...
        <ListGroup className="token-list">
          {data.map((token) => (
            <ListItem key={token.address} token={token} onSelect={props?.onSelect} disabled={token.status === 2}>
              {token.status === 1 ? (
                <span>{formatBalance(token.balance, getUnitFromValue(+token.decimals))}</span>
              ) : (
...
function Manager(props) {
  ...
  const [isRegisteredTokenInvalid, setIsRegisteredTokenInvalid] = useState(false);
  ...
  return (
    <>
      <div className="tab-container">
        <span className={`tab ${active === 0 ? 'active' : ''}`} onClick={onTabClick(0)}>
          {t('crosschain:Register')}
        </span>
        <span className={`tab ${active === 1 ? 'active' : ''}`} onClick={onTabClick(1)}>
      ...
      {active === 0 && (
        <>
          <Form className="register-control">
            <Form.Group controlId="address">
              <Form.Label>{t('crosschain:Token Contract Address')}</Form.Label>
              <Form.Control
                type="text"
              ...
              />
              <Form.Control.Feedback type="invalid">{t('common:Invalid Address')}</Form.Control.Feedback>

              {registeredStatus === 1 && <Form.Text>{t('crosschain:This token has been registered.')}</Form.Text>}

              {registeredStatus === 2 && (
                <Form.Text>
                  {t('crosschain:This token has been registered, switch to upcoming tab to view the register status.')}
                </Form.Text>
              )}
            </Form.Group>
          </Form>
          ...
          {isLoading ? (
            ...
            <div
              ...
              style={{
                display: isRegisteredTokenInvalid || !inputValue ? 'none' : 'block',
              }}
            >
              <h6>{t('Token Info')}</h6>
          ...
          <Button
            ...
            hidden={registeredStatus === 1 || registeredStatus === 2}
          >
            {t('common:Submit')}
          </Button>
        </>
...
function Upcoming(props) {
  const { t } = useTranslation();
  const { loading, allTokens, setAllTokens } = useAllTokens(props.networkType, 2);
  const searchFn = useCallback(tokenSearchFactory(allTokens), [allTokens]);
  const { data, setSearch } = useLocalSearch(searchFn);
      ...
      {loading ? (
        ...
        <ListGroup className="token-list">
          ...
          {data?.map((token, index) => (
            <ListItem key={token.address} token={token} onClick={props.onSelect} disabled={!token.confirmed}>
              <UpcomingTokenState
                token={token}
                onConfirm={() => {
                  ...
                  confirmRegister(token.proof)
                    .then((tx) => {
                      formToast(`Register success transaction hash: ${tx.transactionHash}`);
