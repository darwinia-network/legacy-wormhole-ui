...
import { config, formatBalance, remove0x, textTransform } from '../../page/CrossChain/utils';
...
export default function HistoryRecord({ history }) {
  ...
  if (!!history && !history.length) {
    return <EmptyData className="history-empty" text="crosschain:No Cross-chain transfer history" />;
  }
  ...
  return (
    <>
      ...
      {history.map((item) => {
        ...
        const depositInfo = JSON.parse(item.deposit || '{}');
          ...
          <div className="history-item" key={item.block_hash || item.tx || item.block_timestamp}>
            ...
            {isDeposit || item.isErc20 ? (
              <>
                <div>
                  ...
                  {item.isErc20 ? (
                    ...
                    <p>
                      {t('crosschain:Deposit ID')}: {depositInfo.deposit_id}
                    </p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {item.isErc20 ? (
                    <p>
                      {formatBalance(Web3.utils.toBN(item.amount), 'ether')} {item.symbol} &#40;
                      {t('crosschain:Time')}: {dayjs.unix(item.block_timestamp).format('YYYY/MM/DD')}
                      &#41;
                    </p>
                  ) : (
                    <DepositHistoryDetail amount={item.amount} deposit={depositInfo} />
                  )}
                </div>
              </>
            ) : (
              <div>
                ...
                <p>
                  {formatBalance(Web3.utils.toBN(item.amount), 'ether')} {item.currency?.toUpperCase()}
                </p>
              </div>
            )}
...
// eslint-disable-next-line complexity
function TransferProgress({ from, to, step, hash, hasRelay = false }) {
  const { t } = useTranslation();
    ...
    <div className="transfer-progress">
      <div className="icon-box">
        ...
        <div className={`${getProgress(step, 3)}`}>
          <img
            src={txProgressIcon[`step${getProgress(step, 2, false)}${textTransform(from, 'capitalize')}Icon`]}
            alt="tx"
          ></img>
        </div>
        {hasRelay ? (
          <div className={`${getProgress(step, 3)}`}>
            <img src={txProgressIcon[`step${getProgress(step, 3, false)}RelayIcon`]} alt="tx"></img>
          </div>
        ) : null}
        <div className={`${getProgress(step, 4)}`}>
          <img
            src={txProgressIcon[`step${getProgress(step, 4, false)}${textTransform(to, 'capitalize')}Icon`]}
            alt="tx"
          ></img>
        </div>
      </div>
