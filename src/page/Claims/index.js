...
class Claims extends Component {
  ...
  async queryClaims() {
    ...
    const address =
      networkType === 'eth'
        ? account[networkType]
        : window.tronWeb && window.tronWeb.address.toHex(account[networkType]);
        ...
        <div className={styles.formBox}>
          <div className={`${styles.networkBox} claims-network-box`}>
            ...
            <div className={styles.buttonBox}>
              <Button variant="gray" onClick={this.toResult}>
                {t('page:search')}
              </Button>
              <Button variant="gray" onClick={() => this.toClaims(2)}>
                {t('page:claim')}
              </Button>
            </div>
          </div>
        ...
        <div className={styles.formBox}>
          <div className={`${styles.connectInfoBox} claims-network-box`}>
            <h1>
              ...
              <span>{t('page:Connected to')}：</span>
            </h1>
            <p>{account[networkType]}</p>

            <h1>
              <img alt="" src={labelTitleLogo} />
              <span>{t('page:Snapshot data')}：</span>
            </h1>
            <p>
              {formatBalance(airdropNumber)} RING
              <br />({dayjs.unix(config.SNAPSHOT_TIMESTAMP).format('YYYY-MM-DD HH:mm:ss ZZ')})
            </p>
            ...
            {status === 3 ? (
              <>
                <h1>
                  <img alt="" src={labelTitleLogo} />
                  <span>{t('page:Darwinia Crab Network account')}：</span>
                </h1>
                <p>{darwiniaAddress}</p>
              </>
            ) : null}
          ...
          <div className={styles.formBox}>
            <div className={`${styles.networkBox} claims-network-box`}>
              <Form.Group controlId="darwinaAddressGroup">
                <Form.Label>
                  {t('page:Please enter the account of Darwinia Crab')}{' '}
                ...
                <Form.Control
                  type="text"
                  placeholder={t('page:Darwinia Crab Network account')}
                  value={darwiniaAddress}
                  onChange={(value) => this.setValue('darwiniaAddress', value)}
                />
              </Form.Group>
              <div className={styles.buttonBox}>
                <Button variant="gray" onClick={this.sign}>
                  {t('page:Submit')}
                </Button>
          ...
          <div className={styles.formBox}>
            <div className={`${styles.networkBox} ${styles.signatureBox} claims-network-box`}>
              <Form.Group controlId="signatureGroup">
                <Form.Label>
                  {t('page:Success! Please copy the signature below, and [claim] in Darwinia Wallet')}
                </Form.Label>
                <Form.Control as="textarea" value={JSON.stringify(JSON.parse(signature), undefined, 4)} rows="3" />
              </Form.Group>
              <div className={styles.buttonBox}>
                <CopyToClipboard
                  text={JSON.stringify(JSON.parse(signature), undefined, 4)}
      ...
      <div>
        ...
        <div className={styles.formBox}>
          <div className={`${styles.connectInfoBox} claims-network-box`}>
            ...
            <p>
              {claimAmount.eqn(0) ? formatBalance(airdropNumber) : formatBalance(claimAmount)} RING
              <br />({dayjs.unix(config.SNAPSHOT_TIMESTAMP).format('YYYY-MM-DD HH:mm:ss ZZ')})
            </p>

            <h1>
              <img alt="" src={labelTitleLogo} />
              <span>{t('page:Destination')}：</span>
            </h1>
            <p>{claimTarget || '----'}</p>

            <h1>
              <img alt="" src={labelTitleLogo} />
              <span>{t('page:Claims Result')}：</span>
            </h1>
