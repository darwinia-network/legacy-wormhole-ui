...
export async function approveRingToIssuing(account, hashCallback, confirmCallback) {
  ...
  contract.methods
    .approve(config.ETHEREUM_DARWINIA_ISSUING, '100000000000000000000000000')
    .send({ from: account })
...
export async function getEthereumToDarwiniaCrossChainFee() {
  ...
  const fee = await erc20Contract.methods
...
export async function isNetworkConsistent(expectedNetworkId, id) {
  ...
  const actualId = id ? await Promise.resolve(id) : await window.ethereum.request({ method: 'net_version' });
...
function connectTron(accountsChangedCallback, t) {
  if (typeof window.tronWeb !== 'undefined') {
    ...
    let subscribe = null;
    subscribe = window.tronWeb.on('addressChanged', (wlt) => {
      if (window.tronWeb) {
...
export async function connectNodeProvider(wss) {
  try {
...
function buildInGenesisEth(account, params, callback) {
  ...
  contract.methods
    .transferFrom(account, config['ETHEREUM_DARWINIA_CROSSCHAIN'], params.value, params.toHex)
    .send({ from: account })
    .on('transactionHash', (hash) => {
...
function redeemTokenEth(account, params, callback) {
  ...
  contract.methods
    .transferFrom(account, config['ETHEREUM_DARWINIA_ISSUING'], params.value, params.toHex)
    .send({ from: account })
    .on('transactionHash', (hash) => {
      callback && callback(hash);
    })
...
function redeemDepositEth(account, params, callback) {
  ...
  contract.methods
    .burnAndRedeem(params.depositID, params.toHex)
    .send({ from: account })
    .on('transactionHash', (hash) => {
      callback && callback(hash);
    })
...
async function buildInGenesisTron(account, params, callback) {
  ...
  const contract = await tronwebjs.contract().at(config[`${params.tokenType.toUpperCase()}_TRON_ADDRESS`]);
  const res = contract.methods
    .transferAndFallback(config['TRON_DARWINIA_CROSSCHAIN'], params.value.toString(), params.toHex)
    .send({
...
async function buildInGenesisCrab(account, params, callback) {
  try {
    if (window.darwiniaApi) {
      const injector = await web3FromAddress(account);
      window.darwiniaApi.setSigner(injector.signer);
      const hash = await window.darwiniaApi.tx.crabIssuing.swapAndBurnToGenesis(params.value).signAndSend(account);
      callback && callback(hash);
    }
  } catch (error) {
    console.error(error);
...
async function ethereumBackingLockDarwinia(account, params, callback) {
  try {
    if (window.darwiniaApi) {
      const injector = await web3FromAddress(account);
      window.darwiniaApi.setSigner(injector.signer);
      const hash = await window.darwiniaApi.tx.ethereumBacking
        .lock(params.ring, params.kton, params.to)
        .signAndSend(account);
      callback && callback(hash);
    }
  } catch (error) {
...
export function crossChainFromDarwiniaToEthereum(account, params, callback, t) {
...
export const getDarwiniaToEthereumGenesisSwapInfo = async (params, cb, failedcb) => {
  const json = await wxRequest(params, `${config.DAPP_API}/api/ethereumBacking/locks`);
...
export const getCringGenesisSwapInfo = async (params, cb, failedcb) => {
  const json = await wxRequest(params, `${config.SUBSCAN_API}/api/other/crabissuing`);
  if (json.code === 0) {
    if (!json.data.list || json.data.list.length === 0) {
      cb && cb([]);
      return;
...
export const getErc20BurnsRecords = async ({ row, page, sender }) => {
  ...
  try {
    const {
      data: { data },
    } = await axios.get(`${config.DAPP_API}/api/ethereumIssuing/burns`, { params: { sender, row, page } });
    return data;
  } catch (err) {
    console.warn('%c [ err ]-546', 'font-size:13px; background:pink; color:#bf2c9f;', err);
...
export const getErc20TokenLockRecords = async ({ sender, row, page }) => {
  try {
    const {
      data: { data },
    } = await axios.get(`${config.DAPP_API}/api/ethereumBacking/tokenlock`, { params: { sender, row, page } });

    return data;
  } catch (err) {
    console.warn('%c [ err ]-558', 'font-size:13px; background:pink; color:#bf2c9f;', err);
...
export function encodeMMRRootMessage(networkPrefix, methodID, mmrIndex, mmrRoot) {
  ...
  return registry.createType(
    '{"prefix": "Vec<u8>", "methodID": "[u8; 4; methodID]", "index": "Compact<u32>", "root": "H256"}',
    {
...
export async function ClaimTokenFromD2E(
  { networkPrefix, mmrIndex, mmrRoot, mmrSignatures, blockNumber, blockHeaderStr, blockHash, historyMeta },
  callback,
  fetchingEndCallback,
  t
) {
    ...
    connect(
      ...
      async (_networkType, _account) => {
        if (historyMeta.mmrRoot && historyMeta.best && historyMeta.best > blockNumber) {
          ...
          darwiniaToEthereumAppendRootAndVerifyProof(
            _account,
            {
...
export async function darwiniaToEthereumAppendRootAndVerifyProof(
  account,
  { message, signatures, root, MMRIndex, blockHeader, peaks, siblings, eventsProofStr },
  callback
) {
  const web3js = new Web3(window.ethereum || window.web3.currentProvider);
  ...
  contract.methods
    .appendRootAndVerifyProof(message, signatures, root, MMRIndex, blockHeader, peaks, siblings, eventsProofStr)
...
export async function darwiniaToEthereumVerifyProof(
  account,
  { root, MMRIndex, blockHeader, peaks, siblings, eventsProofStr },
  callback
) {
  const web3js = new Web3(window.ethereum || window.web3.currentProvider);
...
/**
 *
 * @param expectNetworkId  - network id
 * @returns is acutal network id match with expected.
 */
