...
/**
 *
 * @param currentAccount - metamask active account
 * @param networkType - eth or darwinia
 * @returns
...
export const getAllTokens = async (currentAccount, networkType = 'eth') => {
  ...
  return networkType === 'eth' ? await getAllTokensEthereum(currentAccount) : await getAllTokensDvm(currentAccount);
};
...
export const registerToken = async (address) => {
  ...
  if (!isRegistered) {
    const fromAccount = await getMetamaskActiveAccount();
    ...
    const txHash = await register(address).send({ from: fromAccount });
    ...
    return generateRegisterProof(address).subscribe((proof) => proofSubject.next(proof));
  }
};
...
const generateRegisterProof = (address) => {
  return loopQuery(`${config.DAPP_API}/api/ethereumIssuing/register?source=${address}`).pipe(
    switchMap((data) => {
      ...
      const mmrProof = from(getMMRProof(block_num, mmr_index, block_hash)).pipe(
        catchError((err) => {
          console.warn(
...
export const popupRegisterProof = (address) => {
  ...
  return iif(() => !!proof, fromMemo, fromQuery).subscribe((prf) => proofSubject.next(prf));
};

/**
 *
 * @param address - erc20 token address
 * @return status - 0: unregister 1: registered 2: registering
 */
...
export const confirmRegister = async (proof) => {
  ...
  const fromAccount = await getMetamaskActiveAccount();
  const mmrRootMessage = encodeMMRRootMessage(config.D2E_NETWORK_PREFIX, '0x479fbdf9', mmr_index, mmr_root);
...
function getContractWithAddressByNetwork(token, networkType) {
  ...
  const contractAddress = networkType === 'eth' ? config.E2D_BACKING_ADDRESS : config.MAPPING_FACTORY_ADDRESS;
 ...
 * @param recipientAddress - recipient address, ss58 format
 * @param amount - transfer token amount
 * @param currentAccount - metamask current active account
 */
export async function crossSendErc20FromEthToDvm(tokenAddress, recipientAddress, amount, currentAccount) {
  const tx = await backingContract.methods
...
export async function crossSendErc20FromDvmToEth(tokenAddress, recipientAddress, amount, currentAccount) {
