...
import {
  ...
  redeemToken,
  redeemDeposit,
  checkIssuingAllowance,
  approveRingToIssuing,
  getEthereumBankDeposit,
  getEthereumToDarwiniaCrossChainInfo,
  getEthereumToDarwiniaCrossChainFee,
  crossChainFromDarwiniaToEthereum,
  getDarwiniaToEthereumCrossChainFee,
  getDarwiniaToEthereumGenesisSwapInfo,
  substrateAddressToPublicKey,
  ClaimTokenFromD2E,
  getMetamaskActiveAccount,
  getErc20BurnsRecords,
  getErc20TokenLockRecords,
  getMMRProof,
  getMPTProof,
  isNetworkConsistent,
} from './utils';
import {
  canCrossSendToDvm,
  crossSendErc20FromEthToDvm,
  crossSendErc20FromDvmToEth,
  canCrossSendToEth,
  claimErc20Token,
} from './erc20/token';
...
class CrossChain extends Component {
  ...
  toWeiBNMiddleware = (num = '0', unit = 'ether') => {
    try {
      ...
      console.error('🚀 ~ file: index.js ~ line 286 ~ CrossChain ~ error', error);
    }
  };
      ...
      case 'darwinia':
        isNetworkConsistent(status === 2 ? config.DVM_NETWORK_ID : config.ETHEREUM_NETWORK).then((isExpected) => {
          if (isExpected) {
                ...
                connect(
                  ...
                  (_networkType, _account) => {
                    ...
                    if (addressParams && _.find(_account, { address: addressParams })) {
                      ...
                      this.state.account[_networkType] &&
                      _.find(_account, { address: this.state.account[_networkType] })
                    ) {
            ...
            formToast(
              t('Ethereum network type does not match, please switch to {{network}} network in metamask.', {
                network: status === 2 ? config.NETWORK_NAME : config.ETHERSCAN_DOMAIN.name,
              })
            );
          }
        });
  ...
  crossChainFromDarwiniaToEthereum = () => {
    const { networkType, account, recipientAddress, crossChainBalance, crossChainKtonBalance, tokenType, erc20Token } =
      this.state;
    const { t } = this.props;
    ...
    if (tokenType === 'erc20') {
      getMetamaskActiveAccount().then((currentAccount) => {
        return canCrossSendToEth(erc20Token, currentAccount)
          .then((result) => {
            ...
            return crossSendErc20FromDvmToEth(erc20Token.address, recipientAddress, crossChainBalance, currentAccount);
          })
          .then((transferTxHash) => {
  ...
  approveRingToIssuing = () => {
    ...
    if (tokenType === 'erc20') {
      ...
      canCrossSendToDvm(erc20Token, account.eth)
        .then((result) => {
          if (typeof result === 'string') {
            this.setState({ erc20TokenTransferState: { approveTxHash: result } });
          } else {
          ...
          return crossSendErc20FromEthToDvm(erc20Token.address, darwiniaAddress, crossChainBalance, account.eth);
        })
        .then((transferTxHash) => {
          const { erc20TokenTransferState } = this.state;
        ...
        .catch((error) => {
          console.warn('%c [ error ]-499', 'font-size:13px; background:pink; color:#bf2c9f;', error.message);
  ...
  checkFormDarwiniaToEthereum = (unit = 'ether') => {
    ...
    try {
      ...
      if (
        Web3.utils
          .toBN(Web3.utils.toWei(crossChainBalanceText || '0'))
          .add(Web3.utils.toBN(Web3.utils.toWei(crossChainKtonBalanceText || '0')))
          .lten(0)
      ) {
  ...
  async queryClaims() {
    ...
    switch (networkType) {
      case 'eth':
        ...
        const ethereumGenesisInfo = new Promise((resolve) => {
          getBuildInGenesisInfo(
            {
              query: { address },
              method: 'get',
        ...
        // eslint-disable-next-line no-case-declarations
        const ethereumToDarwiniaCrossChainInfo = new Promise((resolve) => {
          getEthereumToDarwiniaCrossChainInfo(
            {
              query: { address },
              method: 'get',
        ...
        Promise.all([ethereumGenesisInfo, ethereumToDarwiniaCrossChainInfo, erc20TransferInfo])
          .then(([genesisHistory, crosschainHistory, erc20CrossChain]) =>
            this.getSymbolMap(erc20CrossChain.list).then((symbolMap) => [
              genesisHistory,
              crosschainHistory,
              erc20CrossChain,
              symbolMap,
          ...
          .then(([genesisHistory, crosschainHistory, erc20CrossChain, symbolMap]) => {
            this.setState({
              history: [
                ...crosschainHistory
                  .concat(
                    erc20CrossChain.list.map((item) => ({ ...item, isErc20: true, symbol: symbolMap[item.source] }))
                  )
                  .map((item) => ({ ...item, isCrossChain: true })),
                ...genesisHistory,
              ],
            });
          })
  ...
  renderHeader = () => {
    ...
    return (
      <>
        ...
        {status === 4 ? (
          <div className={`${styles.stepBox} ${styles.stepResultBox}`}>
            <div className={styles.stepBoxItem}>
              <img alt="" src={step1open} />
              <p>{t('crosschain:step_1')}</p>
            </div>
  ...
  renderDepositItem = (deposit) => {
    ...
    return (
      <p>
        {deposit.amount} RING{' '}
        <span className={styles.depositItem}>
          ({t('crosschain:Deposit ID')}: {deposit.deposit_id} {t('Time')}: {depositStartTime.format('YYYY/MM/DD')} -{' '}
  ...
  renderDepositHistoryDetail = (amount, deposit) => {
    ...
    return (
      <p>
        {formatBalance(Web3.utils.toBN(amount), 'ether')} RING ({t('crosschain:Time')}:{' '}
        {depositStartTime.format('YYYY/MM/DD')} - {depositEndTime.format('YYYY/MM/DD')})
      </p>
    );
  };
  ...
  getDepositByID = (id) => {
    ...
    const r = ethereumDeposits.filter((deposit) => {
      return deposit.deposit_id === parseInt(id, 10);
    });
        ...
        {networkType === 'eth' || networkType === 'tron' ? (
          <div className={styles.formBox}>
            <div className={`${styles.connectInfoBox} claims-network-box`}>
              <h1>
                <img alt="" src={labelTitleLogo} />
                <span>{t('crosschain:Connected to')}：</span>
              </h1>
              ...
              {status === 3 ? (
                <>
                  <h1>
                    <img alt="" src={labelTitleLogo} />
                    <span>{t('crosschain:Darwinia Network account')}：</span>
                  </h1>
                  <p>{darwiniaAddress}</p>
                  <h1>
                    <img alt="" src={labelTitleLogo} />
                    <span>{t('crosschain:Detail')}：</span>
                  </h1>
            ...
            <div className={`${styles.networkBox} claims-network-box`}>
              <Form.Group controlId="darwinaAddressGroup">
                ...
                <Form.Label>
                  {t(
                    tokenType === 'erc20'
                      ? 'crosschain:Please enter Ethereum account or DVM account'
                      : 'crosschain:Please enter the destination account of Darwinia mainnet'
                  )}
                ...
                <Form.Control
                  ...
                  placeholder={t(
                    tokenType === 'erc20'
                      ? 'crosschain:Ethereum account or DVM account'
                      : 'crosschain:Darwinia Network account'
                  )}
                  ...
                  onChange={(value) => {
                    ...
                    if (tokenType !== 'erc20') {
                      ...
                      this.setState({
                        accountValidation: address
                          ? ss58Address
                            ? null
                            : { msg: 'crosschain:This address is not {{type}} smart account' }
                          : null,
                      });
                    } else {
                      this.setState({
                        accountValidation: address
                          ? Web3.utils.isAddress(address)
                            ? null
                            : { msg: 'crosschain:This address is not Ethereum account or DVM account' }
                          : null,
                      });
                    }
                ...
                {!this.state.accountValidation && (
                  <Form.Text className="text-muted">
                    {t(
                      tokenType === 'erc20'
                        ? 'crosschain:Please be sure to fill in the real Ethereum account or DVM account, and keep the account recovery files such as mnemonic properly.'
                        : 'crosschain:Please be sure to fill in the real Darwinia mainnet account, and keep the account recovery files such as mnemonic properly.'
                    )}
                  </Form.Text>
                )}
                      ...
                      onClick={() => {
                        this.setValue(
                          'crossChainBalance',
                          {
                            target: {
                              value: formatBalance(
                                ringBalance.gt(crossChainFee) ? ringBalance.sub(crossChainFee) : Web3.utils.toBN(0),
                                'ether'
                              ),
                            },
                          },
                          this.toWeiBNMiddleware,
                          this.setRingBalanceText
                        );
                      }}
                    >
                      <Form.Control
                        type="number"
                        placeholder={`${t('crosschain:Balance')} : ${formatBalance(
                          this.state[`${tokenType}Balance`],
                          'ether'
                        )}`}
                        autoComplete="off"
                        value={crossChainBalanceText}
                        onChange={(value) =>
                          this.setValue('crossChainBalance', value, this.toWeiBNMiddleware, this.setRingBalanceText)
                        }
                      />
                    </InputRightWrap>
                  </>
                  ...
                  <>
                    ...
                    <InputRightWrap
                      ...
                      onClick={() => {
                        this.setValue(
                          ...
                          {
                            target: {
                              value: formatBalance(this.state.erc20Token?.balance, this.state.erc20Token?.decimals),
                            },
                          },
                          this.toWeiBNMiddleware,
                          this.setRingBalanceText
                    ...
                    >
                      <Form.Control
                        type="number"
                        placeholder={`${t('crosschain:Balance')} : ${formatBalance(
                          this.state.erc20Token?.balance,
                          this.state.erc20Token?.decimals
                        )}`}
                        autoComplete="off"
                        value={crossChainBalanceText}
                        onChange={(value) =>
                  ...
                  <>
                    ...
                    <InputRightWrap
                      ...
                      onClick={() => {
                        this.setValue(
                          'crossChainBalance',
                          { target: { value: formatBalance(this.state[`${tokenType}Balance`], 'ether') } },
                          this.toWeiBNMiddleware,
                          this.setRingBalanceText
                        );
                      }}
                  ...
                  <>
                    ...
                    >
                      ...
                      <Dropdown.Menu className="super-colors">
                        {ethereumDeposits &&
                          ethereumDeposits.length > 0 &&
                          ethereumDeposits.map((item) => {
                            return (
                              <Dropdown.Item
                                active={item.deposit_id === currentDepositID}
                                key={item.deposit_id}
                                eventKey={item.deposit_id}
                              >
                                {this.renderDepositItem(item)}
                              </Dropdown.Item>
                            );
                          })}
                      </Dropdown.Menu>
              ...
              <div className={styles.buttonBox}>
                {isAllowanceIssuing || tokenType === 'deposit' ? (
                  <Button
                    disabled={(tokenType !== 'erc20' && !isSufficientFee && tokenType !== 'deposit') || isApproving}
                    variant="color"
                    onClick={() => {
                      if (tokenType === 'erc20') {
                  ...
                  <Button disabled={isApproving} variant="color" onClick={this.approveRingToIssuing}>
                    {isApproving ? t('crosschain:Approving') : t('crosschain:Approve')}
                  </Button>
                )}
                <Button variant="outline-purple" onClick={() => this.goBack(1)}>
                  {t('crosschain:Back')}
                </Button>
              </div>
            </div>
            ...
            <div className={`${styles.networkBox} claims-network-box`}>
              <Form.Group controlId="darwiniaAddressGroup" className={styles.formInputBox}>
                  ...
                  <>
                      ...
                      onClick={() => {
                        this.setValue(
                          'crossChainBalance',
                          {
                            target: {
                              value: formatBalance(
                                ringBalance.gte(Web3.utils.toBN(2000000000).add(crossChainFee))
                                  ? ringBalance.sub(Web3.utils.toBN(2000000000)).sub(crossChainFee)
                                  : Web3.utils.toBN(0),
                                'gwei'
                              ),
                            },
                          },
                          (value) => this.toWeiBNMiddleware(value, 'gwei'),
                          this.setRingBalanceText
                        );
                      }}
                      label="RING"
                      inputText={crossChainBalanceText}
                      placeholder={`${t('crosschain:Balance')} : ${formatBalance(ringBalance, 'gwei')} RING`}
                      onChange={(value) =>
                        this.setValue(
                          'crossChainBalance',
                          value,
                          (val) => this.toWeiBNMiddleware(val, 'gwei'),
                          this.setRingBalanceText
                        )
                      }
                    ...
                    <InputWrapWithCheck
                      ...
                      onClick={() => {
                        this.setValue(
                          'crossChainKtonBalance',
                          { target: { value: formatBalance(ktonBalance, 'gwei') } },
                          (value) => this.toWeiBNMiddleware(value, 'gwei'),
                          this.setKtonBalanceText
                        );
                      }}
                      label="KTON"
                    ...
                    <Form.Control
                      as="select"
                      value={account[networkType]}
                      onChange={(value) =>
                        this.setCurrentAccount('darwinia', value, async (acc) => {
                          const balances = await getTokenBalance('darwinia', acc);

                          this.setState({
                            ringBalance: Web3.utils.toBN(balances[0]),
                            ktonBalance: Web3.utils.toBN(balances[1]),
                            darwiniaAddress: convertSS58Address(acc),
                          });
                        })
                      }
                    >
                      {account[`${networkType}List`]?.map((item) => (
                        <option key={item.address} value={item.address}>
                          {convertSS58Address(item.address, middleScreen)} - {item.meta.name}
                        </option>
                      ))}
                    </Form.Control>
                  </>
                ...
                <Form.Control
                  type="text"
                  autoComplete="off"
                  placeholder={t('crosschain:Ethereum account')}
                  value={recipientAddress}
                  onChange={(value) => this.setValue('recipientAddress', value, null)}
                />
                <Form.Text className="text-muted">
                  {t(
                    'crosschain:Please make sure to fill in an Ethereum network account that does not belong to the exchange'
                  )}
                </Form.Text>

                <FormTip text={[t('crosschain:d2e crosschain gas tip 1'), t('crosschain:d2e crosschain gas tip 2')]} />
              </Form.Group>
              <div className={styles.buttonBox}>
                <Button variant="color" onClick={this.crossChainFromDarwiniaToEthereum}>
                  {t('crosschain:Submit')}
                </Button>
                <Button variant="outline-purple" onClick={() => this.goBack(1)}>
                  {t('crosschain:Back')}
        ...
        return (
          <div className={styles.formBox}>
            <div className={`${styles.networkBox} claims-network-box`}>
              <Form.Group controlId="darwinaAddressGroup">
                <Form.Label>
                  {t('crosschain:Please select Darwinia mainnet destination account')}{' '}
                  <a href={this.renderHelpUrl()} target="_blank" rel="noopener noreferrer">
                    <img alt="" className={styles.labelIcon} src={helpLogo} />
                  </a>{' '}
                </Form.Label>
                <Form.Control
                  ...
                  value={account[networkType]}
                  onChange={(value) =>
                    this.setCurrentAccount('crab', value, async (acc) => {
                      const balances = await getTokenBalance('crab', acc);
                      this.setState({
                        ringBalance: Web3.utils.toBN(balances[0]),
                        darwiniaAddress: convertSS58Address(acc),
                      });
                ...
                <Form.Control as="select" value={tokenType} onChange={(value) => this.setValue('tokenType', value)}>
                  ...
                  onClick={() => {
                    this.setValue(
                      'crossChainBalance',
                      {
                        target: {
                          value: formatBalance(
                            ringBalance.gte(Web3.utils.toBN(2000000000))
                              ? ringBalance.sub(Web3.utils.toBN(2000000000))
                              : Web3.utils.toBN(0),
                            'gwei'
                          ),
                        },
              ...
              <div className={styles.buttonBox}>
                <Button variant="color" onClick={this.buildInGenesis}>
                  {t('crosschain:Submit')}
                </Button>
                <Button variant="outline-purple" onClick={() => this.goBack(1)}>
                  {t('crosschain:Back')}
  ...
  step1 = () => {
    ...
    return (
      <div>
        ...
        <div className={styles.formBox}>
          <div className={`${styles.networkBox} claims-network-box`}>
            <Form.Group controlId="networkSelectGroup">
              ...
              <Form.Control
                as="select"
                value={`${networkType}-${to}`}
                onChange={(value) =>
                  this.setValue(
                    'networkType',
                    value,
                    (val) => {
                      return this.splitNetworkStr(val)[0];
                    },
                    (data) => {
                      const netType = this.splitNetworkStr(data);
                      history.replace({
                        hash: `#${parseChain(netType[0])}`,
                      });
                      this.setState({
                        from: netType[0],
                        to: netType[1],
                      ...
                      onChangePath({
                        from: parseChain(netType[0]),
                        to: parseChain(netType[1]),
                      });
                    }
                  )
                }
            ...
            <div className={styles.buttonBox}>
              <Button variant="outline-purple" onClick={this.toResult}>
                {t('crosschain:search')}
              </Button>
          ...
          <div className={styles.formBox}>
            <div className={styles.stepRoadMap}>
              ...
              <div className={`${styles.stepRoadMapItem} ${styles.stepRoadMapItemDone}`}>
                <div>
                  <p>
                    <img src={roadmapStatus0} alt="end"></img>
                    <span>{t('crosschain:Phase 1')}</span>
                  </p>
                  <p>{t('crosschain:In progress')}</p>
                </div>
                <p>
                  {t(
                    'crosschain:The RING in genesis swaps will arrive after launching the Darwinia mainnet and will be sent to the destination account by Genesis Block.'
                  )}
                </p>
              </div>
              <div className={styles.stepRoadMapItem}>
                <div>
                  <p>
                    <img src={roadmapStatus1} alt="start"></img>
                    <span>{t('crosschain:Phase 2')}</span>
                  </p>
                  <p>2021</p>
                </div>
                <p>
                  {t(
                    'crosschain:Cross-chain swaps at this stage will arrive immediately (network delays may occur),but only support One-way swaps to the Darwinia main network.'
                  )}
                </p>
              </div>
              <div className={`${styles.stepRoadMapItem} ${styles.stepRoadMapItemDone}`}>
                <div>
                  <p>
                    <img src={roadmapStatus2} alt="start"></img>
                    <span>{t('crosschain:Phase 3')}</span>
                  </p>
                  <p>2021</p>
                </div>
                <p>
                  {t(
                    'crosschain:Cross-chain swaps at this stage will arrive immediately (network delays may occur), and support two-way or multi-way swaps.'
                  )}
                </p>
              </div>
            </div>
        ...
        {networkType === 'eth' || networkType === 'darwinia' ? (
          <div className={styles.formBox}>
            <div className={styles.stepRoadMap}>
              ...
              <div className={`${styles.stepRoadMapItem} ${styles.stepRoadMapItemDone}`}>
                <div>
                  <p>
                    <img src={roadmapStatus0} alt="end"></img>
                    <span>{t('crosschain_ethtron:Phase 1')}</span>
                  </p>
                  <p>{t('crosschain:In progress')}</p>
                </div>
                <p>
                  {t(
                    'crosschain_ethtron:The cross-chain transfers at this stage will arrive after launching the Darwinia mainnet and will be sent to the destination account by Genesis Block'
                  )}
                </p>
              </div>
              <div className={`${styles.stepRoadMapItem}  ${styles.stepRoadMapItemDone}`}>
                <div>
                  <p>
                    <img src={roadmapStatus0} alt="end"></img>
                    <span>{t('crosschain_ethtron:Phase 2')}</span>
                  </p>
                  <p>{'2020 Q4'}</p>
                </div>
                <p>
                  {t(
                    'crosschain_ethtron:Cross-chain transfers at this stage will arrive immediately (network delays may occur),but only support One-way transfers to the Darwinia main network'
                  )}
                </p>
              </div>
              <div className={`${styles.stepRoadMapItem}`}>
                <div>
                  <p>
                    <img src={roadmapStatusActive0} alt="end"></img>
                    <span>{t('crosschain_ethtron:Phase 3')}</span>
                  </p>
                  <p>{'2021 Q1'}</p>
                </div>
                <p>
                  {t(
                    'crosschain_ethtron:Cross-chain transfers at this stage will arrive immediately (network delays may occur), and support two-way or multi-way transfers'
                  )}
                </p>
              </div>
            </div>
          ...
          <div className={styles.formBox}>
            <div className={`${styles.networkBox} ${styles.hashBox} claims-network-box`}>
              <Form.Group controlId="signatureGroup">
                <Form.Label>
                  {t(
                    'crosschain:The transaction has been sent, please check the transfer progress in the cross-chain history.'
                  )}
                </Form.Label>
              ...
              <div className={styles.buttonBox}>
                <Button variant="color" onClick={this.toResult}>
                  {t('crosschain:search')}
                </Button>
                <Button variant="outline-purple" onClick={() => this.goBack(1)}>
                  {t('crosschain:Back')}
    ...
    return (
      <div>
        ...
        <div className={styles.formBox}>
                ...
                <Form.Row>
                  {crabAndDarwiniaHistory.isManual ? (
                    <Form.Group as={Col}>
                      <Form.Control
                        onChange={(value) =>
                          this.setCurrentAccount(networkType, value, async (acc) => {
                            const params = new URLSearchParams();
                            const { hash } = this.props.location;

                            if (acc) {
                              params.append('address', acc);
                            } else {
                              params.delete('address');
                            }
                            this.props.history.replace({ hash, search: params.toString() });

                            this.queryClaims();
                          })
                        }
                        type="text"
                        placeholder={t('crosschain:Please input the Darwinia address to be queried')}
                      />
                    </Form.Group>
                  ) : (
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (crabAndDarwiniaHistory.isManual) {
                          ...
                          account[`${networkType}List`][0] &&
                            this.setCurrentAccount(
                              networkType,
                              { target: { value: account[`${networkType}List`][0].address } },
                              async (acc) => {
                                const params = new URLSearchParams();
                                const { hash } = this.props.location;
          ...
          <div className={styles.buttonBox}>
            <Button variant="outline-gray" onClick={() => this.goBack(1)}>
              {t('crosschain:Back')}
            </Button>
          </div>
        </div>
    ...
    return (
      <>
        ...
        {this.renderDarwiniaToEthereumHistory(
          ...
          historyMeta,
          (item) =>
            `${formatBalance(Web3.utils.toBN(item.ring_value), 'gwei')} RING ${
              item.kton_value !== '0' ? ' / ' + formatBalance(Web3.utils.toBN(item.kton_value), 'gwei') + ' KTON' : ''
            }`
        )}
        {this.renderDarwiniaToEthereumHistory(
          erc20History,
      ...
      <>
        {history
          ? history.map((item) => {
              ...
              return (
                <div className={styles.historyItem} key={item.extrinsic_index}>
                  <div>
              ...
              return (
                <div className={styles.historyItem} key={item.extrinsic_index}>
                  ...
                  <div>
                    <h3>{t('crosschain:Amount')}</h3>
                    <p>
                      {formatBalance(Web3.utils.toBN(item.amount), 'gwei')} CRING -&gt;{' '}
                      {formatBalance(Web3.utils.toBN(item.amount).divn(100), 'gwei')} RING
                    </p>
                  </div>
                  <div>
                    <h3>{t('crosschain:Destination account')}</h3>
      ...
      <div className={styles.transferProgress}>
        <div className={styles.iconBox}>
          ...
          <div className={`${this.renderProgress(step, 3)}`}>
            <img
              src={txProgressIcon[`step${this.renderProgress(step, 2, false)}${textTransform(from, 'capitalize')}Icon`]}
              alt="tx"
            ></img>
          </div>
          {hasRelay ? (
            <div className={`${this.renderProgress(step, 3)}`}>
              <img src={txProgressIcon[`step${this.renderProgress(step, 3, false)}RelayIcon`]} alt="tx"></img>
            </div>
          ) : null}
          <div className={`${this.renderProgress(step, 4)}`}>
            <img
              src={txProgressIcon[`step${this.renderProgress(step, 4, false)}${textTransform(to, 'capitalize')}Icon`]}
              alt="tx"
            ></img>
          </div>
        </div>
        <div className={styles.titleBox}>
          ...
          <div className={`${this.renderProgress(step, 2)}`}>
            ...
            <Button
              ...
              variant="outline-purple"
              target="_blank"
              href={this.renderExplorerUrl(hash.from.tx, hash.from.chain)}
            >
              {t('crosschain:Txhash')}
            </Button>
          </div>
          ...
          <div className={`${this.renderProgress(step, 4)}`}>
            ...
            {step >= 4 && hash.to && hash.to.tx ? (
              <Button
                ...
                variant="outline-purple"
                target="_blank"
                href={this.renderExplorerUrl(hash.to.tx, hash.to.chain)}
              >
                {t('crosschain:Txhash')}
              </Button>
            ) : null}
      ...
      <div>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title id="contained-modal-title-vcenter">{t('crosschain:Transaction feedback')}</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <p>
              {t(
                'crosschain:The transaction has been sent, the transaction results can be tracked through the block explorer. No need to send the transaction repeatedly before the transaction result comes out.'
              )}
              <a rel="noopener noreferrer" target="_blank" href={this.renderExplorerUrl(d2eModalData.hash, 'eth')}>
                {t('crosschain:View on etherscan')}
              </a>
            </p>
          </Modal.Body>
          <Modal.Footer>
            <Button variant="purple" onClick={() => this.setModalShow(false)}>
              {t('crosschain:Close')}
            </Button>
          </Modal.Footer>
        </Modal>
        ...
        <Erc20Token
          show={this.state.isRegisterTokenModalDisplay}
        ...
        <Alert
          show={!!this.state.erc20TokenTransferState}
        ...
        >
          ...
          {this.state.erc20TokenTransferState?.approveTxHash && (
            <p>
              {t('crosschain:Approve transaction hash - {{hash}}', {
                hash: this.state.erc20TokenTransferState?.approveTxHash,
              })}
            </p>
          )}
          {this.state.erc20TokenTransferState?.transferTxHash && (
            <p>
              {t('crosschain:Transfer transaction hash - {{hash}}', {
                hash: this.state.erc20TokenTransferState?.transferTxHash,
              })}
            </p>
          )}
        </Alert>
