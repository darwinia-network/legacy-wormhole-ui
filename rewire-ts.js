 ...
 * @return {Array}
 */
const ruleChildren = (rule) => rule.use || rule.oneOf || (Array.isArray(rule.loader) && rule.loader) || [];
...
function rewireTypescript(config, env, typescriptLoaderOptions = {}) {
  ...
  config.resolve.extensions = (config.resolve.extensions || []).concat(['.web.ts', '.ts', '.tsx']);
